...
#endif
#include "fire_query.h"

#define MAXCLIENT   10
#define MAXPENDING 	5
#define DB_NAME "fireproject"
...
void* SensorInterruptLoop(void* vp)
{
	...
	{
#ifndef TEST_MODE
		_fire_sensor = get_fire_sensor();
		_gas_sensor = get_gas_sensor();
#endif

		sprintf(_query, QUERY_INSERT_SENSOR_DATA, _fire_sensor, _gas_sensor);
		printf("%s : write to DB - %s\n", __func__, _query);
		iot_insert_data_from_db(_query);

		sprintf(_query, QUERY_INSERT_SENSOR_CHECK, _fire_sensor, _gas_sensor);
		printf("%s : write to DB - %s\n", __func__, _query);
		iot_insert_data_from_db(_query);
		sprintf(_query, QUERY_INSERT_ACTUATOR_VALUE, _use_pump, _use_buzzer, _use_fan, _use_rgbled);
		printf("%s : write to DB - %s\n", __func__, _query);
		iot_insert_data_from_db(_query);
#ifndef TEST_MODE
		sprintf(_query, QUERY_INSERT_ACTUATOR_CHECK, get_pump_functionality(), get_buzzer_functionality(), get_fan_functionality(), get_rgbled_functionality());
		printf("%s : write to DB - %s\n", __func__, _query);
		iot_insert_data_from_db(_query);
#endif
	...
	while (1)
	{
		...
		case FIRE:
			if (_message->_command == GET_STATUS)
			{
				...
				iRet = sprintf(ucSBuff, "%d|%d|%d,%d,%d,%d,%d,%d", _message->_apptype, _message->_command, _fire_sensor, _gas_sensor, get_pump_functionality(), get_buzzer_functionality(), get_fan_functionality(), get_rgbled_functionality());
			...
			else if (_message->_command == GET_SETTING)
			{
				...
				iRet = sprintf(ucSBuff, "%d|%d|%s,%d,%d,%d,%d", _message->_apptype, _message->_command, _result, _use_pump, _use_buzzer, _use_fan, _use_rgbled);
				free(_result);
				write(stMyInfo.iSock, ucSBuff, iRet);
			}
			...
			else if (_message->_command == FIRE_PUMP)
			{
				memset(_data, 0x0, sizeof(Data));
				iRet = parse_data(_message->_data, _data);
			...
			else if (_message->_command == FIRE_BUZZER)
			{
				memset(_data, 0x0, sizeof(Data));
				iRet = parse_data(_message->_data, _data);
				_use_buzzer = _data->_data1;
				set_buzzer_active(_data->_data1);
				iRet = sprintf(ucSBuff, "%d|%d", _message->_apptype, _message->_command);
				write(stMyInfo.iSock, ucSBuff, iRet);

			}
			else if (_message->_command == FIRE_FAN)
			{
				memset(_data, 0x0, sizeof(Data));
				iRet = parse_data(_message->_data, _data);
			...
			else if (_message->_command == FIRE_LED)
			{
				memset(_data, 0x0, sizeof(Data));
				iRet = parse_data(_message->_data, _data);
